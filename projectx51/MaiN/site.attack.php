...
?><form method=post><table cellpadding=1 cellspacing=1 border=1><tr><tr><th>Attacks</th><th>Actions</th></tr><?php
...
print '<tr';if(empty($bg)){
	?> bgcolor="#1234561"<?php
	$bg=1;
}else{
	$bg='';
}print '><td><a title="Attack Damage '.number_format($attack_damage).'
Attack Radius '.number_format($attack_radius).'
Attack Time '.number_format($attack_time).'
Attack Cost $'.number_format($attack_cost).'
">'.$weapons[$i].'</a></td><td>';
if (($row->{"x1$i"} - $current_time) >= 1) {
print ''.dater($row->{"x1$i"});
} else {
