...
for ($x=$min_x; $x<=$max_x; $x++) {
	...
	$whatever='';
	print '<form method=post><input type=hidden name=xx value="'.$x.'"><input type=hidden name=yy value="'.$y.'"><td align="center" valign="center" width=65 height=75 bgcolor="'.color_me($x,$y).'">';
...
if ($lresult = mysqli_query ($link, "SELECT * FROM `$tbl_location` WHERE (`x`='$x' and `y`='$y') ORDER BY `id` DESC LIMIT 1")) {
	...
	mysqli_free_result ($lresult);
		if ($lrow->mid == $row->id) {
			if ($row->money >= $cost_upgrade and $lrow->x0 <= 999) {
				$whatever='Upgrade';
			}else{
				//NO MONEY
			}
