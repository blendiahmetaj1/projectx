...
if($row->onoff == 4 or $row->onoff == 2){
		//PLAYERS VISIBLE
if($presult = mysqli_query ($link, "SELECT `id`,`timer` FROM `$tbl_members` WHERE `x`='$x' and `y`='$y' and `id`!='$row->id'   and `timer`>=($current_time-$online_show) ORDER BY `id` DESC LIMIT 1")){
...
if($row->onoff == 5 or $row->onoff == 2){
		//KINGDOMS
if($kresults = mysqli_query ($link, "SELECT `id` FROM `$tbl_kingdoms` WHERE `x`='$x' and `y`='$y' ORDER BY `id` DESC LIMIT 1")) {
...
<td valign=center align=center>
<div style="font-size: 0px; line-height: 0%; width: 0px;border: 3px solid <?php print $col_buildings;?>;"> </div></td><td><font size=-2><a href="?map=3">Buildings</a>
</font>
</td></tr>
<tr><td valign=center align=center>
...
<td valign=center align=center>
<div style="font-size: 0px; line-height: 0%; width: 0px;border: 3px solid <?php print $col_kingdom;?>;"> </div></td><td><font size=-2><a href="?map=5">Kingdoms</a>
</font>
</td></tr></table><?php
//BIG MAP
?>